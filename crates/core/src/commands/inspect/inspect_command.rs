...
use crate::commands::inspect::format::DIVIDER;
use crate::prelude::*;
...
impl InspectCommand {
    ...
    pub(crate) fn execute_cli(&self) -> Result<bool, Failure<InspectAction>> {
        let output = get_details(&self.arg.path)?;
        println!("{output}");
        Ok(true)
    }
}
...
pub(crate) fn get_details(dir: &Path) -> Result<String, Failure<InspectAction>> {
    let (properties, tags) = get_details_split(dir)?;
    Ok(format!("{properties}{DIVIDER}{tags}"))
}

/// Get track details split into properties table and per-track tags.
pub(crate) fn get_details_split(dir: &Path) -> Result<(String, String), Failure<InspectAction>> {
